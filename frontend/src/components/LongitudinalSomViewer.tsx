    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-gray-900/60 border border-gray-800 rounded-3xl max-w-4xl mx-auto my-8 shadow-2xl backdrop-blur-xl">
        ...
        <h3 className="text-2xl font-black text-white tracking-tight mb-2">
          Longitudinal SOM Analysis (Mapas Evolutivos Encadenados)
        </h3>
        <p className="text-sm text-gray-400 text-center max-w-xl mb-8 leading-relaxed">
          Para subperiodos de <strong>5 años o más</strong>, el sistema entrena un mapa para cada ventana temporal usando el protocolo <em>Warm-Start</em> de Kohonen: los pesos del periodo anterior inicializan el siguiente SOM, acelerando la convergencia en fase de refinamiento (20% iteraciones) y garantizando la coherencia topológica.
        </p>
          ...
          <div className="w-full bg-gray-950/80 border border-gray-800 rounded-2xl p-6 mb-8 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 pt-2 text-xs">
              <div className="bg-gray-900/60 p-3 rounded-xl border border-gray-800">
                <span className="text-gray-500 block mb-1">Malla SOM</span>
                <span className="text-white font-bold">{config.rows} × {config.cols} ({config.rows * config.cols} neuronas)</span>
              </div>
              <div className="bg-gray-900/60 p-3 rounded-xl border border-gray-800">
                <span className="text-gray-500 block mb-1">Iteraciones Base</span>
                <span className="text-white font-bold">{config.iterations} épocas</span>
              </div>
              <div className="bg-gray-900/60 p-3 rounded-xl border border-gray-800">
                <span className="text-gray-500 block mb-1">Iteraciones Refinamiento</span>
                <span className="text-indigo-300 font-bold">{Math.max(10, Math.round(config.iterations * 0.2))} épocas (Warm-Start)</span>
              </div>
            </div>
          </div>
        ) : (
          <div className="p-4 bg-amber-950/40 border border-amber-800/60 rounded-2xl text-xs text-amber-300 mb-6">
            ⚠️ No se han generado matrices por subperiodo. Asegúrate de marcar "Generate Temporal Sequences" y definir un subperiodo de 5 años en la pestaña de Redes Bibliométricas.
          </div>
        )}
        ...
        >
          {isTraining ? (
            ...
            <>
              ...
              <span>Entrenar SOMs Longitudinales</span>
            </>
          )}
        </button>
      </div>
      ...
      <div className="flex items-center justify-between bg-gray-900/80 border border-gray-800 rounded-2xl p-4 shadow-xl backdrop-blur-md">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-lg font-black text-white tracking-tight">
                Análisis Longitudinal SOM (Mapas Evolutivos)
              </h2>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Evolución temporal continua con alineación espacial de cuadrantes temáticos
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center bg-gray-950 border border-gray-800 rounded-xl p-1 space-x-1">
          ...
          >
            ...
            <span>Deriva del Conocimiento (ΔW)</span>
          </button>
          ...
          <div className="lg:col-span-3 bg-gray-900/60 border border-gray-800 rounded-3xl p-6 flex flex-col justify-between shadow-2xl">
            ...
            <div className="flex items-center justify-between border-b border-gray-800/80 pb-4 mb-4">
              <div className="flex items-center space-x-3">
                <span className="text-sm font-bold text-gray-300">Periodo Activo:</span>
            ...
            <div className="flex-1 flex items-center justify-center min-h-[420px]">
              {activeMap ? renderHexGrid(activeMap) : (
                <div className="text-gray-500 text-xs">No hay datos para este mapa</div>
              )}
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-900/60 border border-gray-800 rounded-3xl p-5 shadow-xl space-y-4">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center space-x-2">
                ...
                <span>Metadatos del Periodo</span>
              </h4>
                ...
                <div className="space-y-2.5 text-xs">
                  <div className="flex justify-between py-1 border-b border-gray-800/60">
                    ...
                    <span className="font-bold text-indigo-300">
                      {activeMap.training_phase === 'base_full' ? 'Base (Ordenamiento Global)' : 'Warm-Start (Refinamiento)'}
                    </span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-800/60">
                    <span className="text-gray-400">Épocas Ejecutadas:</span>
                    <span className="font-bold text-white">{activeMap.iterations} épocas</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-800/60">
                    <span className="text-gray-400">Términos Activos:</span>
                  ...
                  {activeMap.drift_from_prev && (
                    <div className="flex justify-between py-1 border-b border-gray-800/60">
                      <span className="text-gray-400">Deriva Media (ΔW):</span>
            ...
            <div className="bg-gray-900/60 border border-gray-800 rounded-3xl p-5 shadow-xl space-y-4">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center space-x-2">
                ...
                <span>
                  {selectedNeuron !== null ? `Neurona #${selectedNeuron}` : 'Inspector de Neurona'}
                </span>
              </h4>
              ...
              {selectedNeuron !== null && activeMap ? (
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="bg-gray-950 p-2.5 rounded-xl border border-gray-800">
                      <span className="text-gray-500 block text-[10px]">Frecuencia BMU</span>
                  ...
                  <div>
                    <span className="text-[11px] font-bold text-gray-400 block mb-1.5">
                      Términos Mapeados ({activeMap.mappedLabels?.[selectedNeuron]?.length || 0}):
                    </span>
                    <div className="max-h-48 overflow-y-auto space-y-1 pr-1">
                      ...
                      {(!activeMap.mappedLabels?.[selectedNeuron] || activeMap.mappedLabels[selectedNeuron].length === 0) && (
                        <span className="text-xs text-gray-600 italic">Neurona vacía en este periodo</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <p className="text-xs text-gray-500 leading-relaxed">
                  Haz clic en cualquier hexágono de la malla para inspeccionar los términos asociados, cluster y evolución en este periodo.
                </p>
              )}
            </div>
          </div>
            ...
            return (
              <div key={p} className="bg-gray-900/60 border border-gray-800 rounded-3xl p-5 flex flex-col shadow-xl">
                ...
                <div className="pt-3 border-t border-gray-800/60 mt-3 flex justify-between text-[11px] text-gray-400">
                  <span>Términos: <strong className="text-white">{pMap.mappedLabels?.reduce((acc: number, cur: any[]) => acc + cur.length, 0) || 0}</strong></span>
                  {pMap.drift_from_prev && (
                    <span>Deriva: <strong className="text-amber-400">{pMap.drift_from_prev.mean_drift?.toFixed(3)}</strong></span>
                  )}
                </div>
              </div>
            );
        ...
        <div className="space-y-6">
          <div className="p-4 bg-indigo-950/30 border border-indigo-500/30 rounded-2xl flex items-start space-x-3">
            ...
            <div className="text-xs text-indigo-200 leading-relaxed">
              <strong>Deriva del Conocimiento (&Delta;W_t = ||W_t - W_&#123;t-1&#125;||_2):</strong> Mide la magnitud de desplazamiento de los centroides neuronales entre periodos sucesivos. Las regiones con color más cálido/intenso representan frentes conceptuales con mayor dinamismo y reconfiguración disciplinar.
            </div>
          </div>
                  ...
                  <div className="flex items-center justify-between border-b border-gray-800 pb-3 mb-4">
                    ...
                    <div className="flex space-x-2 text-[10px]">
                      <span className="px-2 py-0.5 bg-gray-950 rounded border border-gray-800 text-gray-300">
                        Media: <strong>{driftMetric.mean_drift?.toFixed(4)}</strong>
                      </span>
                      <span className="px-2 py-0.5 bg-gray-950 rounded border border-gray-800 text-amber-300">
                        Máx: <strong>{driftMetric.max_drift?.toFixed(4)}</strong>
                      </span>
                    </div>
                  </div>
      ...
      {activeSubTab === 'migration' && (
        <div className="bg-gray-900/60 border border-gray-800 rounded-3xl p-6 shadow-2xl space-y-4">
          <h3 className="text-sm font-bold text-white uppercase tracking-wide">
            Evolución de Términos a través de los Subperiodos
          </h3>
          <p className="text-xs text-gray-400">
            Seguimiento de la neurona BMU asociada a cada término en cada periodo temporal.
          </p>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-gray-800 text-gray-400">
                  <th className="py-2.5 px-3">Término / Concepto</th>
                  {periods.map(p => (
                    <th key={p} className="py-2.5 px-3">{p} (Neurona / BMU)</th>
                  ))}
